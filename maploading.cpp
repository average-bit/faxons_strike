...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(db.isOpen())
    {
        ...
        QSqlQuery query6;
        query6.prepare (QString ("SELECT [%1] FROM NPCS WHERE rowid = :val").arg(Object_ID));
        ...
        while (query6.next())
        {
            Opcodes::Old_ID = query6.value(0).toString().toUpper();
            Opcodes::RespawnID = query6.value(0).toString().toUpper();
        }
    ...
    rotate2 = 0;
        ...
        for(rotateZ = 1; rotateZ < tablesizeZ + 1; rotateZ++)
        {
    ...
    qDebug() << "Opcodes::MaintoonsCharID" << Opcodes::MaintoonsCharID;
    ...
    for (rotate = 1; rotate < tablesize + 1; rotate++) //use the number of rows of npcs table
    {
        ...
        while (query4.next())
        {
            ...
            QVector <QVector<float>> CurrentZoneCentralVector = checkthezone::VectorOfWorlds.at(newWorld);
    ...
    for (rotate3 = 1; rotate3 <= tablesize3 + 1; rotate3++) //equal cause db starts on row 1
    {
    ...
    for (rotate4 = 1; rotate4 <= tablesize4 + 1; rotate4++) //1 cause db starts on row 1
    {
    ...
    for(rotate6 = 1; rotate6 <=  tablesize6 + 1; rotate6++) //equal cause db starts on row 1
    {
    ...
    for(rotate7 = 1; rotate7 <= tablesize7 + 1; rotate7++) //1 cause db starts on row 1
    {
