...
QByteArray Quests::SendoutSpell(QByteArray ID_IP_PORT, QString ResultID)
{
    ...
    QString Target = IN_SendoutSpell.CharSelectID_Main;

    int index = ResultID.indexOf("#");

    QString SpellGraphic = ResultID.mid(index+1,ResultID.size());
...
QByteArray Quests::SendPopUps(QByteArray ID_IP_PORT, QString ResultID)
{
    ...
    QString Text = Quests::PopUpID_PopUpText_Map.value(ResultID);
...
QByteArray Quests::QuestMenus(QByteArray ID_IP_PORT, QByteArray Clients_Last_MessageNumber, QByteArray Clients_Last_FB_MessageNumber, QByteArray opholder, QString QuestsID)
{
    SendPacketOut.clear();
    ...
    qDebug() << "IN_QuestMenus.CheckPocketsNow" << IN_QuestMenus.CheckPocketsNow;
    qDebug() << "client_return1" << client_return1;
    qDebug() << "tempvectmap" << tempvectmap;

    IN_QuestMenus.MenuID_QuestID.insert(IN_QuestMenus.CurrentQuestMenuID,QuestsID);

    QVector<QString> PocketCheckVector;

    if(client_return1 == 0 && IN_QuestMenus.CheckPocketsNow == true)
    {
        IN_QuestMenus.CheckPocketsNow = false;

        packetparsing::IPandPort_AllvariablesMap.insert(ID_IP_PORT,IN_QuestMenus);

        PocketCheckVector = CheckYoPockets(ID_IP_PORT,opholder,IN_QuestMenus.CurrentQuestID);
        ...
        if(PocketCheckVector.isEmpty() == false)
        {
            for(int re = 0; re < PocketCheckVector.size(); re++)
            {
                ...
                QByteArray PopUpsReturn = SendPopUps(ID_IP_PORT,CurrentQuestMenuID);
    ...
    if(PocketCheckVector.isEmpty() == true)
    {
        ...
        for(int re = 0; re < ResultsVector.size(); re++)
        {
            ...
            if(ResultID.contains("IC_"))//IC_BCBC808000#2
            {
                ...
                QuestsID = IN_QuestMenus.MenuID_QuestID.value(IN_QuestMenus.CurrentQuestMenuID);

                if(IN_QuestMenus.QuestsID_MenusID_ItemsID_QuantityVectorMap.contains(QuestsID))
                {
                    tempCheckVect = IN_QuestMenus.QuestsID_MenusID_ItemsID_QuantityVectorMap.value(QuestsID);
                }
                ...
                qDebug() << "IC_QuestsIDx" << QuestsID;
                qDebug() << "IC_CurrentQuestMenuIDx" << CurrentQuestMenuID;
                qDebug() << "IC_ItemsIDx" << ItemsID;
                qDebug() << "IC_Quantityx" << Quantity;

                IN_QuestMenus.QuestsID_MenusID_ItemsID_QuantityVectorMap.insert(QuestsID,tempCheckVect);
            }
            ...
            if(ResultID.contains("TC_"))//TC_150
            {
                ...
                IN_QuestMenus.QuestsID_MenusID_TunarQuantityVectorMap.insert(QuestsID,tempCheckVect);
            }
    ...
    if(SendPacketOut.isEmpty() == false)
    {
        ...
        Send_FB = IN_QuestMenus.serverid1 + IN_QuestMenus.clientid1 + "xxxx01BF543213" + IN_QuestMenus.SessionID + "03" +
        ...
        qDebug() << "----------------------------------------------------------";
        qDebug() << dateTimeString;
        qDebug() << "WorldGreeting_Menus_and_popups";
...
QByteArray Quests::WelcomeGreetings(QByteArray ID_IP_PORT)
{
    ...
    {
        {"Freeport","P_09000000"},{"Qeynos","P_0B000000"},{"Surefall Glade","P_0C000000"},
    ...
    ResultsVector.append("P_0A000000");
...
QVector<QString> Quests::CheckYoPockets(QByteArray ID_IP_PORT, QByteArray opholder, QString QuestsID)
{
    packetparsing::packetvars IN_CheckYoPockets;
    IN_CheckYoPockets = packetparsing::IPandPort_AllvariablesMap.value(ID_IP_PORT);

    QVector<QString> QMIQ_TempVect = IN_CheckYoPockets.QuestsID_MenusID_ItemsID_QuantityVectorMap.value(QuestsID);
    ...
    if(QMIQ_TempVect.isEmpty() == false)
    {
        ...
        for(int re = 0; re < QMIQ_TempVect.size(); re+=3)
        {
            CurrentQuestMenuID = QMIQ_TempVect.at(re);
            ItemsID = QMIQ_TempVect.at(re+1);
            Quantity = QMIQ_TempVect.at(re+2);

            qDebug() << "IN_CheckYoPockets.QuestsID" << QuestsID;
            ...
            if(IN_CheckYoPockets.InventorySlots_ItemIDs.contains(ItemsID))
            {
                ...
                if(qvalNeeded > qvalHave)
                {
                    ...
                    if(!ReturnResultsVector.contains("P_20000000"))
                    {
                        ReturnResultsVector.append("P_20000000");
                ...
                else
                {
                    ...
                    if(checkAll == true && re == QMIQ_TempVect.size())
                    {
                        IN_CheckYoPockets.QuestsID_MenusID_ItemsID_QuantityVectorMap.remove(QuestsID);
                    }
                }
            }
            else
    ...
    QVector<QString> QMTQ_TempVect = IN_CheckYoPockets.QuestsID_MenusID_TunarQuantityVectorMap.value(QuestsID);

    if(QMTQ_TempVect.isEmpty() == false)
    {
        QString CurrentQuestMenuID = "";
        QString TunarQty = "";
        ...
        if(QMTQ_TempVect.isEmpty() == false)
        {
            CurrentQuestMenuID = QMTQ_TempVect.at(0);
            TunarQty = QMTQ_TempVect.at(1);
        }

        qDebug() << "IN_CheckYoPockets.CurrentQuestMenuID" << CurrentQuestMenuID;
        ...
        if(Tunar_U_Need > Tunar_U_Have)
        {
            ...
            ReturnResultsVector.append("P_1F000000");
        ...
        else
        {
            ...
            if(checkAll == true)
            {
                IN_CheckYoPockets.QuestsID_MenusID_TunarQuantityVectorMap.remove(QuestsID);
            }
        }
    }
    ...
    qDebug() << "IN_QuestMenus.checkAll" << checkAll;
...
QByteArray Quests::RemoveQuestLog(QByteArray ID_IP_PORT, QString ResultID)
{
    ...
    ADD_OR_SUB sendto;
    QString Returned = sendto.FBSize(Opcode);
