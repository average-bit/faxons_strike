    ...
    if(IN_Opcodes.OpcodeReturn.isEmpty() == false)
    {
        do
        {
            ...
            if(SendToID != IN_Opcodes.CharSelectID)
            {
                ...
                qDebug() << "Player Interactions" << Description;
...
QByteArray Opcodes::CheckTheOpcode(QByteArray ClientID, QByteArray ClientsIP, QByteArray ClientsPort)
{
    ...
    qDebug() << "IN_Opcodes.CurrentQuestMenuID_7" << IN_Opcodes.CurrentQuestMenuID;
    ...
    while(iterOp.hasNext())
    {
        ...
        if(IN_Opcodes.opcoderesult1 == "2D00" || IN_Opcodes.opcoderesult1 == "2B00")
        {
            ...
            if(IN_Opcodes.opcoderesult1 == "2B00")//Create Character
            {
                ...
                if(!Opcodes::PlayerNames.contains(textout))
                {
                    ...
                    qDebug() << "IN_Opcodes.CharSelectID_2 = " << IN_Opcodes.CharSelectID;
        ...
        if(IN_Opcodes.opcoderesult1 == "0400")
        {
            ...
            QString NPCsID = QString::fromUtf8(opholder.mid(12,8));
            ...
            if(Quests::QuestGiversID_NameMap.contains(NPCsID))
            {
                QString NPCsName = Quests::QuestGiversID_NameMap.value(NPCsID);
                int PacketID = Quests::QuestGiversNames_IDS_Map.value(NPCsName);

                qDebug() << "NPCsName" << NPCsName;
                qDebug() << "PacketID" << PacketID;

                qDebug() << "Quests::FakeFlag" << Quests::FakeFlag;
                ...
                if(Quests::QuestValuesVectorMap.contains(PacketID))
                {
                    ...
                    QString QNPCsID = "FAF30400";
                    QString QLevel = "01";
                    QString QRace = "HUM";
                    QString QClass = "MAG";
                    QString QStartingCity = "Freeport";

                    QVector<QString> tempvect1 =
                    {
                        QStartingCity,QRace,QClass
                    };
                    ...
                    QMap<QString,QMap<QString,QVector<QString>>> LocationClassRaceQuestGiverMap =
                    {
                        {QNPCsID,{tempMap}}
                    };
                    ...
                    qDebug() << "";
                    qDebug() << "Race1" << Race1;
                    qDebug() << "Class1" << Class1;
                    qDebug() << "Level1" << Level1;
                    qDebug() << "StartingCity" << StartingCity;

                    QMap<QString,QVector<QString>> Level_ConditionsVectorMap = Quests::QuestGiversID_Level_LocationClassRaceMap.value(NPCsID);

                    QVector<QString> ConcitionsVect = Level_ConditionsVectorMap.value(Level1);

                    for(int re = 0; re < ConcitionsVect.size(); re++)
                    {
                        QString Condition = ConcitionsVect.at(re);
                        qDebug() << "Condition" << Condition;
                    }

                    QMap<QVector<int>,QString> RangeAndQuestIDvectMap =  Quests::LocationQuestID_QuestVectorsMap.value(Race1+Class1+StartingCity);

                    qDebug() << "RangeAndQuestIDvectMap" << RangeAndQuestIDvectMap;

                    IN_Opcodes.QuestGiversID = NPCsID;

                    QByteArray SendPacketOut = "";
                    QVector<QString>tempvect = Quests::QuestValuesVectorMap.value(PacketID);

                    qDebug() << "tempvect" << tempvect;

                    QVector<QString> QuestIDVect = Quests::NPCsID_QuestIDsVectorMap.value(NPCsID);

                    qDebug() << "QuestIDVect" << QuestIDVect;


                    QMap<QVector<int>,QString> QuestID_QuestVectorsMap;

                    for(int re = 0; re < QuestIDVect.size(); re++)
                    {
                        QString QuestID = QuestIDVect.at(re);

                        qDebug() << "QuestID" << QuestID;

                        QVector<int> tempRangevect = RangeAndQuestIDvectMap.key(QuestID);

                        qDebug() << "tempRangevect" << tempRangevect;

                        QuestID_QuestVectorsMap.insert(tempRangevect,QuestID);
                    }


                    QString Name = NPCsName;


                    QMap<QString,QMap<QVector<int>,QString>> QGname_QuestID_QuestVectorsMap =
                    {
                        {Name,QuestID_QuestVectorsMap}
                    };


                    QMap<QVector<int>,QString> QuestID_QuestVectorsMapTemp = QGname_QuestID_QuestVectorsMap.value(Name);

                    QString QuestsID = "";

                    QMapIterator<QVector<int>,QString> iter (QuestID_QuestVectorsMapTemp);
                    while (iter.hasNext())
                    {
                        iter.next();
                        QVector<int> ValueRange = iter.key();

                        int low = ValueRange.at(0);
                        int high = ValueRange.at(1);

                        qDebug() << "low" << low;
                        qDebug() << "Quests::FakeFlag" << Quests::FakeFlag;
                        qDebug() << "high" << high;

                        if(low <= Quests::FakeFlag && Quests::FakeFlag <= high)
                        {
                            QuestsID = iter.value();
                        }
                    ...
                    QVector<QString> tempvectqr;
                    QVector<QString> PocketCheckVector;
                    QVector<QString> tempPopUpResultVect;

                    if(QuestsID.isEmpty() == false)
                    {
                        IN_Opcodes.CurrentQuestID = QuestsID;

                        int index = tempvect.indexOf(QuestsID);

                        qDebug() << "index/7" << index/7;

                        Description = tempvect.at(index+1);
                        StartingCity1 = tempvect.at(index+2);
                        Faction = tempvect.at(index+3);
                        Race = tempvect.at(index+4);
                        Class = tempvect.at(index+5);
                        Level  = tempvect.at(index+6);

                        qDebug() << "Quests::QuestsIDs_ResultsVectorMap" << Quests::QuestsIDs_ResultsVectorMap;

                        tempvectqr = Quests::QuestsIDs_ResultsVectorMap.value(QuestsID);

                        qDebug() << "QuestsID" << QuestsID;
                        qDebug() << "Description" << Description;
                        qDebug() << "StartingCity" << StartingCity1;
                        ...
                        qDebug() << "tempvectqr" << tempvectqr;

                        if(tempvectqr.isEmpty() == false)
                        {
                            if(Quests::FakeFlag == 1)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 2)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(1);
                            }

                            if(Quests::FakeFlag == 3)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 4)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 6)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 8)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 9)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 10)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(1);
                            }

                            if(Quests::FakeFlag == 11)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 12)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(1);
                            }

                            if(Quests::FakeFlag == 13)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(0);
                            }

                            if(Quests::FakeFlag == 14)
                            {
                                IN_Opcodes.CurrentQuestMenuID = tempvectqr.at(1);
                            }
                        ...
                        qDebug() << "IN_Opcodes.MenuID_QuestID1" << IN_Opcodes.MenuID_QuestID;

                        qDebug() << "Quests::FakeFlag" << Quests::FakeFlag;
                        qDebug() << "IN_Opcodes.CurrentQuestMenuIDx" << IN_Opcodes.CurrentQuestMenuID;



                        qDebug() << "";

                        IN_Opcodes.Quest = true;


                        Quests sendto;
                        bool empty = true;

                        if(IN_Opcodes.QuestsID_MenusID_ItemsID_QuantityVectorMap.isEmpty() == false)
                        {
                            empty = false;
                        }

                        if(IN_Opcodes.QuestsID_MenusID_TunarQuantityVectorMap.isEmpty() == false)
                        {
                            empty = false;
                        }
                        ...
                        if(empty == false)
                        {
                            IN_Opcodes.CheckPocketsNow = true;
                        }

                        qDebug() << "IN_Opcodes.CheckPocketsNow1" << IN_Opcodes.CheckPocketsNow;

                        packetparsing::IPandPort_AllvariablesMap.insert(ID_IP_PORT,IN_Opcodes);

                        IN_Opcodes = packetparsing::IPandPort_AllvariablesMap.value(ID_IP_PORT);

                        if(IN_Opcodes.CurrentQuestMenuID.contains("M_"))
                        {
                        ...
                        {
                            QString CurrentQuestMenuID = IN_Opcodes.CurrentQuestMenuID;
                            QString ItemsID = "";
                            QString Quantity = "";
                            QString TunarQty = "";


                            tempPopUpResultVect = Quests::PopUpID_PopUpResltsVectorMap.value(CurrentQuestMenuID);
                            ...
                            for(int re = 0; re < tempPopUpResultVect.size(); re++)
                            {
                                ...
                                if(ResultID.contains("IC_"))//IC_BCBC808000#2
                                {
                                    QVector<QString> tempCheckVect;

                                    QuestsID = IN_Opcodes.MenuID_QuestID.value(IN_Opcodes.CurrentQuestMenuID);

                                    if(IN_Opcodes.QuestsID_MenusID_ItemsID_QuantityVectorMap.contains(QuestsID))
                                    {
                                        tempCheckVect = IN_Opcodes.QuestsID_MenusID_ItemsID_QuantityVectorMap.value(QuestsID);
                                    }

                                    tempCheckVect.append(CurrentQuestMenuID);
                                    ItemsID = ResultID.mid(3,10);
                                    Quantity = ResultID.mid(14,ResultID.size());
                                    tempCheckVect.append(ItemsID);
                                    tempCheckVect.append(Quantity);

                                    qDebug() << "IC_QuestsID" << QuestsID;
                                    qDebug() << "IC_CurrentQuestMenuID" << CurrentQuestMenuID;
                                    qDebug() << "IC_ItemsID" << ItemsID;
                                    qDebug() << "IC_Quantity" << Quantity;

                                    IN_Opcodes.QuestsID_MenusID_ItemsID_QuantityVectorMap.insert(QuestsID,tempCheckVect);
                                }

                                if(ResultID.contains("TC_"))//TC_150
                                {
                                    QVector<QString> tempCheckVect;
                                    tempCheckVect.append(CurrentQuestMenuID);
                                    TunarQty = ResultID.mid(3,ResultID.size());
                                    tempCheckVect.append(TunarQty);

                                    qDebug() << "QuestsID" << QuestsID;
                                    qDebug() << "CurrentQuestMenuID" << CurrentQuestMenuID;
                                    qDebug() << "TunarQty" << TunarQty;

                                    IN_Opcodes.QuestsID_MenusID_TunarQuantityVectorMap.insert(QuestsID,tempCheckVect);
                                }
                            }
                            ...
                            {
                                qDebug() << "IN_Opcodes.CheckPocketsNow2" << IN_Opcodes.CheckPocketsNow;

                                QuestsID = IN_Opcodes.MenuID_QuestID.value(IN_Opcodes.CurrentQuestMenuID);
                                qDebug() << "IN_Opcodes.QuestsID" << QuestsID;

                                PocketCheckVector = sendto.CheckYoPockets(ID_IP_PORT,opholder,QuestsID);
                        ...
                        tempPopUpResultVect = Quests::PopUpID_PopUpResltsVectorMap.value(IN_Opcodes.CurrentQuestMenuID);

                        qDebug() << "tempPopUpResultVectx" << tempPopUpResultVect;

                        for(int re = 0; re < tempPopUpResultVect.size(); re++)
                        {
                            QString ResultID = tempPopUpResultVect.at(re);

                            qDebug() << "ResultIDx" << ResultID;

                            if(!ResultID.contains("xp_") && !ResultID.contains("IC_") && !ResultID.contains("TC_"))
                            {
                        ...
                        qDebug() << "PocketCheckVectorx" << PocketCheckVector;
                        ...
                        for(int re = 0; re < PocketCheckVector.size(); re++)
                        {
                            ...
                            if(ResultID.contains("IT_") || ResultID.contains("IG_"))
                            {
                                qDebug() << "ResultID" << ResultID;
                            ...
                            if(ResultID.contains("TT_") || ResultID.contains("TG_"))
                            {
                                qDebug() << "ResultID" << ResultID;
                            ...
                            if(ResultID.contains("QLa_"))
                            {
                                ...
                                qDebug() << "Quests::FakeFlag_QLa" << Quests::FakeFlag;
                            }
                            ...
                            if(ResultID.contains("QLr_"))
                            {
                                ...
                                qDebug() << "Quests::FakeFlag_QLr" << Quests::FakeFlag;
                            }
                        }
                    ...
                    {
                        IN_Opcodes.CurrentQuestMenuID = "P_5C000000";
                        QString ResultID = "P_5C000000";
                        qDebug() << "ResultID" << ResultID;
                    ...
                    qDebug() << "IN_Opcodes.Questx" << IN_Opcodes.Quest;

                    qDebug() << "IN_Opcodes.SendPacketOut" << SendPacketOut;
                    ...
                    if(SendPacketOut.isEmpty() == false)
                    {
                        ...
                        Send_FB = IN_Opcodes.serverid1 + IN_Opcodes.clientid1 + "xxxx01BF543213" + IN_Opcodes.SessionID + "03" +
                                Servers_Last_MessageNumber + Clients_Last_MessageNumber + Clients_Last_FB_MessageNumber + SendPacketOut;
            ...
            else
            {
                ...
                if(coach_packet != "NULL")
                {
                    ...
                    IN_Opcodes.FBPacketDescriptionVector.append("Coach_Window, ");
                }
                else
                {
        ...
        if(IN_Opcodes.opcoderesult1 == "3200") //button presses on the spell bar
        {
                    ...
                    (slot1 == "9" && IN_Opcodes.slot_9_trigger == true))
            {
                ...
                spell_recovery = "b100595E0300595E030000000000004a85c79e";
                    ...
                    (slot1 == "9" && IN_Opcodes.cooldown_9 ==  true))
            {
                ...
                //                timer2->setObjectName(IN_Opcodes.spell_str1);
                ...
                main_spell = "b100595E03005a18060000e80300002E6E280A";
        ...
        if(IN_Opcodes.opcoderesult1 == "3300") //memorize spell
        {
            ...
            QByteArray bar_swap = "B400010000000205";
        ...
        if(IN_Opcodes.opcoderesult1 == "3500") //Menu result
        {
            ...
            QString NPCsID = "";// fix this


            if((CoachmanIDsVector.contains(NPCsID) && Quests::FakeFlag < 4) || (CoachmanIDsVector.contains(NPCsID) && Quests::FakeFlag > 4))
            {
            ...
            else
            {
                ...
                qDebug() << "IN_Opcodes.CurrentQuestMenuID_8" << IN_Opcodes.CurrentQuestMenuID;

                Quests sendto;
                sendto.QuestMenus(ID_IP_PORT,Clients_Last_MessageNumber,Clients_Last_FB_MessageNumber,opholder,IN_Opcodes.CurrentQuestMenuID);

                IN_Opcodes = packetparsing::IPandPort_AllvariablesMap.value(ID_IP_PORT);

                qDebug() << "IN_Opcodes.CurrentQuestMenuID_13" << IN_Opcodes.CurrentQuestMenuID;
            }
        }
        ...
        if(IN_Opcodes.opcoderesult1 == "3A00")
        {
            ...
            //            spellunmemorize(); //calling IN_Opcodes.unmemorize
        }
        ...
        if(IN_Opcodes.opcoderesult1 == "4B00")//sell items
        {
            ...
            BackToDec = ConvertBack.BackToDec(IN_Opcodes.Cash_on_hand);
            int Cash_on_hand1 = BackToDec.toInt(nullptr,10);
            //qDebug() << "OldWalletValue" << IN_Opcodes.Cash_on_hand1;
            ...
            if(LevelStatusIs == "GOOD" && ClassStatusIs == "GOOD" && RaceStatusIs == "GOOD")
            {
                ...
                if(Items_Type == "EQuipment")
                {
                    if(IsItemEquipped == "NO")// EQuipt Item
                    {
                        ...
                        IN_Opcodes.EquipmentSlotsUsed.remove(EQuipSlot);
                                ...
                                (EQuipSlot == "15") || (EQuipSlot == "06") || (EQuipSlot == "07"))
                        {
                            ...
                            if(WeaponGraphic == "00000000"  && EQuipSlot != "06" && EQuipSlot != "07")//this means it is Armor
                            {
                                ...
                                if(EQuipSlot == "08") // if it is in 2forearms slot
                                {
                                    ...
                                    {
                                        IN_Opcodes.EquipmentSlotsUsed.remove("06");
                                    ...
                                    {
                                        IN_Opcodes.EquipmentSlotsUsed.remove("07");
                            ...
                            if(ArmorGraphic == "00" && EQuipSlot != "15" && EQuipSlot != "06" && EQuipSlot != "07")//this means it is Weapons/Held...
                            {
                                ...
                                for(int i = 0; i < WeaponCloseIfOpen.size(); i++)
                                {
                                    ...
                                    if(FreeUsed == "USED")// check if slots are in use
                                    {
                                        ...
                                        current_c9.replace(Position,Length,"00000000");
                                        IN_Opcodes.EquipmentSlotsUsed.remove(CheckSlot);
                            ...
                            ///////////////////////////////////////////////////////////////////////////////////////////////////////////
                            if(EQuipSlot == "06")//if its is in right forearm slot
                            {
                                ...
                                if(FreeUsed == "USED")//check if 2forearms is used
                                {
                                    ...
                                    IN_Opcodes.EquipmentSlotsUsed.remove("08");
        ...
        if(IN_Opcodes.opcoderesult1 == "2506")
        {
            ...
            else// group is not full
            {
                ...
                else
                {
                    ...
                    //            FB1409002506020120000A0000004661786F6E6174696F6E
        ...
        if(ReformGroup == true)
        {
            ...
            else
            {
                ...
                //file.write("Regroup ");
                //file.write("\n");
                //file.write("TempGrpVect.size() = " + QString::number(TempGrpVect.size()).toUtf8());
                //file.write("\n");
                ...
                for(int grp = 0; grp < TempGrpVect.size(); grp+=2)
                {
                    ...
                    //file.write("Group_Member = " + TempGrpVect.at(grp).toUtf8() + " " + QString::number(grp).toUtf8());
                    //file.write("\n");
                ...
                if(IN_Opcodes.opcoderesult1 == "1500")
                {
                    ...
                    if(IN_Opcodes.isitOpen == false)
                    {
                        ...
                        if(MobIDs_ItemIDs_Map.contains(MobsID))
                        {
                            ...
                            {
                                Items_ID = MobIDs_ItemIDs_Map.value(MobsID);
                                ...
                                Items_ID = GlobalLootVector.at(RandomLootNumber1);
                        ...
                        if(IN_Opcodes.FakeLootVector.isEmpty() == false)
                        {
                            ...
                            {
                                Items_ID = IN_Opcodes.FakeLootVector.at(re);
    ...
    qDebug() << "IN_Opcodes.CurrentQuestMenuID_14" << IN_Opcodes.CurrentQuestMenuID;
